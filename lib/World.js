...
var createTree = require('./Tree.js');

var World = function World(){
  this._board = this.generateWorld();
  this._players = {};
}

World.prototype.generateWorld = function generateWorld(){
  var b = new FixedArray(100,100,0);
  b.forEach(function(gameObj, index){
    if (gameObj === 0){
      var r = Math.round( Math.random() * 100 );
      if ( r < 10 ){
        b.set(index.x, index.y, createTree());
      }
    }
  });
  return b;
...
World.prototype.gather = function gather(action){
  var p = action.player;

  var trees = this._board.getNeighbours(p.row, p.col).filter(function(n){ return n.name === 'tree'; });
  if (trees.length > 0){
    trees[0].giveApples(p);
    console.log(p.name+' gathered 1 apple successfully!');
  }else{
    throw new Error(p.name+' cannot '+p.raw+' since there are no nearby trees!');
  }
}
